...
function hookRunInNewContext(matcher, transformer, opts) {
    ...
    vm.runInNewContext = function (code, sandbox, filename) {
        ...
        var result = originalRunInNewContext(ret.code, sandbox, filename);

        var subcoverage = ctx[opts.coverageVariable];
        for (var flnm in subcoverage) {
          var glb = global[opts.coverageVariable][flnm];
          if (glb === undefined) {
            global[opts.coverageVariable][flnm] = subcoverage[flnm];
          } else {
            global[opts.coverageVariable][flnm] = objectUtils.mergeFileCoverage(
              glb, subcoverage[flnm]);
          }
        }
...
function hookRunInContext(matcher, transformer, opts) {
    opts = opts || {};
    var fn = transformFn(matcher, transformer, opts.verbose);
    vm.runInContext = function (code, ctx, filename) {
        if (filename === undefined) {
          filename = "?";
        }
        var ret = fn(code, filename);
        var result = originalRunInContext(ret.code, ctx, filename);
